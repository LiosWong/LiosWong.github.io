...
top: 10000
---
最近在做把各个应用某些业务节点的日志通过kafka consumer消费并写入es，考虑到频繁的写入es会造成应用资源、网络开销的浪费，设计上为批量写入es。该应用场景比较通用，考虑把该生产-消费模型
抽成公共组件。
...
该组件实现具体功能可以概括为：任务不断的被放入阻塞队列中，线程池会不断轮训队列，当任务数达到阈值时，会批量实现具体业务需求，并在程序退出时，最大化保证任务不丢失。
...
public class QueueTaskExecutor {
    ...
    public QueueTaskExecutor setExecutorService(ExecutorService executorService) {
...
public class QueueTaskConfig {
    ...
    public QueueTaskExecutor queueTaskExecutor() {
        return new QueueTaskExecutor()
                ...
                .setExecutorService(ThreadPoolUtil.ES_BULK_EXECUTOR)
                .setMaxElements(traceMsgBulkSize)
                .setTaskName(DRIVER_TRACE_QUEUE_TASK_NAME);
    }
